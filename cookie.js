    ...
    function generateAnalyticsDashboard(language = 'en') {
        ...
        <div class="analytics-dashboard">
            ...
            <div class="time-based-stats">
                ${generateTimeStatSection(lang.statsLast1Day, last1Day, lang)}
                ${generateTimeStatSection(lang.statsLast7Days, last7Days, lang)}
                ${generateTimeStatSection(lang.statsLast30Days, last30Days, lang)}
            </div>
        </div>`;
    }

    function generateTimeStatSection(title, data, lang) {
        return `
        <div class="time-stat">
            <h4>${title}</h4>
            <div class="stat-bars">
                ${Object.entries(data).map(([date, dayData]) => {
                    const dayTotal = dayData.accepted + dayData.rejected + dayData.custom;
                    const dayAcceptedPercent = dayTotal > 0 ? (dayData.accepted / dayTotal) * 100 : 0;
                    const dayRejectedPercent = dayTotal > 0 ? (dayData.rejected / dayTotal) * 100 : 0;
                    const dayCustomPercent = dayTotal > 0 ? (dayData.custom / dayTotal) * 100 : 0;
                    ...
                    <div class="stat-bar-container">
                        ...
                        <div class="stat-bar-legend">
                            <span>${dayData.accepted} ${lang.statsAccepted}</span>
                            <span>${dayData.custom} ${lang.statsCustom}</span>
                            <span>${dayData.rejected} ${lang.statsRejected}</span>
                        </div>
                    </div>`;
                }).join('')}
            </div>
